...
\begin{document}
	...
	On peut calculer $L_{15}$ de la façon suivante :\\
	$L_{15} = L_{16} \oplus F( R_{15}\oplus K_{16}) = L_{15} \oplus  F( R_{15}\oplus K_{16})  \oplus  F( R_{15}\oplus K_{16})$
	On a donc $2^{9}$ possibilités pour $K_{16}$, et $2^{5}$ possibilités pour $L_{15}^{*}$ ce qui nous fait $2^{14}$. Il faut ensuite refaire le même type de recherche exhaustive que celle décrite dans la question 2 pour chaque couple $(L_{15}, L_{15}^{*})$ possible, ce qui amène la complexité à : $3*2^{10}*2^{14} = 2^{24}$
	...
	On peu déjà écrire $L_{16} = (L_{13}\oplus F(R_{13} \ K_{14}))\oplus F(R_{15}\oplus K_{16})$\\
	$R_{14} = L_{13} \oplus F(R_{13}\oplus K_{14})\\
	$
